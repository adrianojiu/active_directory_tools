 ...
 ForEach-Object {
   ...
   if ($FailCount -ne 1) {
    ...
    Invoke-Command -ComputerName $ComputerName -ScriptBlock { 
      ...
      if ($svc -eq $null)
            { write-host "$Ghostname Zabbix Agent nao instalado." -ForegroundColor Red 
             }
      
      else {  Start-Service $svc
              Start-Sleep 5
              Write-Host $Ghostname $svc.name $svc.Status -ForegroundColor Blue 
            }
      } 
   }  
